...
def cityEachPageUrl(cityUrl,header):
        ...
        try:
                #print(cityUrl)
                #print(header)
                rq = urllib.request.Request(cityUrl,headers=header)
...
def eachInfo(eachCityPage,header):
        eachInfoUrl = []
        pattern = re.compile(r'<a class="info-tit" href="(.*?)"')
        pattern2 = re.compile(r'<a class="info-tit"  target="_blank" href="(.*?)"')
        try:
                rq = urllib.request.Request(eachCityPage,headers=header)
                eachInfoPage = urllib.request.urlopen(rq)
...
for city in cityUrl:
        ...
        for cityPage in cityPageUrl:
                eachInfoUrls = eachInfo(cityPage,header)
